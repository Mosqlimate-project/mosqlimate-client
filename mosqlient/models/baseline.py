...
def plot_predictions(df_preds: pd.DataFrame, title: str = '') -> None:
    ...
    ax.fill_between(df_preds.date, df_preds.lower,
                    df_preds.upper, color='tab:orange', alpha=0.3)
...
def plot_forecast(df_for: pd.DataFrame, df_train: pd.DataFrame, last_obs: int) -> None:
    ...
    ax.fill_between(df_for.date, df_for.lower, df_for.upper,
                    color='tab:red', alpha=0.3)

    ax.plot([df_train.index[-1], df_for.date[0]], [df_train[f'data'].values[-1],
            df_for.preds.values[0]], ls='--', color='black')
...
class Arima:
    ...
    def train(self, train_ini_date: str, train_end_date: str):
        ...
        df_train = df_train.loc[(df_train.index >= pd.to_datetime(train_ini_date)) & (
    ...
    def predict_in_sample(self, plot: bool = True) -> pd.DataFrame:
        ...
        df_in_sample = get_prediction_dataframe(
            preds_in_sample, df_train.index, self.boxcox)
        ...
        df_in_sample['data'] = self.boxcox.inverse_transform(df_in_sample['data'])[
            0]
    ...
    def predict_out_of_sample(self, horizon: int, end_date: str, plot=True) -> pd.DataFrame:
        ...
        while (pd.Timestamp(dates[-1]) < pd.to_datetime(end_date)):
            ...
            preds = model.update(
                df.loc[dates[0]: dates[-1]]).predict(horizon, return_conf_int=True)
