...
def plot_preds(
    data, df_preds, alpha = 0.9, data_col="casprov", color_palette="Set2", linestyle="-"
):
    """
    ...
    validate_df_preds(df_preds, alpha = alpha)
    ...
    for model in models:
        ...
        ax.fill_between(
            preds_.date,
            preds_[f'lower_{int(100*alpha)}'],
            preds_[f'upper_{int(100*alpha)}'],
            color=color_map[model],
            alpha=0.1,
        )
