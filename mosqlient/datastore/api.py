...
class Infodengue:
    ...
    def get(cls, **kwargs):
        ...
        gen_data = get_datastore('datastore', 'infodengue',
        params)
...
class Copernicus:
    ...
    def get(cls, **kwargs):
        ...
        gen_data = get_datastore('datastore', 'climate',
        params)
    ...
    def __init__(self, **kwargs):
        for k, v in kwargs.items():
            ...
            if k == 'uf': 
                if len(v) != 2: 
                    raise ValueError(
                        f"Invalid 'uf' parameter. It should be a two-letter value"
                    )
                
            if k == 'geocode': 
                if len(str(v)) != 7: 
                    raise ValueError(
                        f"Invalid 'geocode' parameter. It should be a seven numbers code"
                    )
